...
/*   Updated: 2022/07/07 12:45:13 by amarini-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	std::cout << YELLOW << " CR-CREATING" << END << CYAN << " STACK" << END
		<< YELLOW << " BZZ Z-ZZOMBIE :\n" << END;
	...
	std::cout << YELLOW << " DELETING" << END << CYAN <<" S-SSSTACK"<< END
		<< YELLOW <<" ZZZZOMBIIIIIIE :\n" << END;
	delete zombie_stack;
	std::cout << CYAN << " STACK " << END << GREEN << "BZZ-BZZZZT Z-Z-ZOMBIE DELETEEEEED\n" << END;

	std::cout << YELLOW <<"\n CRRRREATING BZZZ" << END << CYAN <<" HEAP" << END
		<< YELLOW << " BZZZZZ-ZOMBIIIE :\n" << END;
	randomChump(heap);
	std::cout << CYAN << " HEAP" << END << GREEN << " Z-ZZZ-ZZZZOMBIE CR-CRRRRR-CREATED\n" << END;
	std::cout << CYAN << " HEAP" << END << RED << " ZZ-ZZZOMBIE DIEDD.. FAILUUUUURE BZZZZ"
		<< END << std::endl;

	std::cout << RED << "\n EXPERIMENT RESULT BZZZZ :\n" << END;
	std::cout << YELLOW << " BZZZ" << END << CYAN << " STACK " << END << YELLOW
		<< "ZOMBIE STAY ALIVE UNTIL DESTROYED\n" << END;
	std::cout << CYAN << " HEAP " << END << YELLOW
